...
namespace ht    // HT API
{
	...
	extern "C" __declspec(dllexport) ht::HtHandle * open(		//  открыть HT
		const wchar_t* fileName,					// имя файла
		bool someFlag			// true если открыть fileMapping; false если открыть файл; по умолчанию false
	);							// != NULL успешное завершение  
	...
	extern "C" __declspec(dllexport) BOOL removeOne      // удалить элемент в хранилище
	(
